...
REM Kill any Python processes running jarvis_main.py
tasklist /FI "IMAGENAME eq python.exe" 2>NUL | find /I /N "python.exe">NUL
if "%ERRORLEVEL%"=="0" (
    echo Searching for Jarvis instances...
    
    REM Kill all Python processes that might be running Jarvis
    powershell -Command "Get-Process python -ErrorAction SilentlyContinue | Where-Object {$_.CommandLine -like '*jarvis_main.py*'} | Stop-Process -Force"
